...
get_snps<-function(samples){
	...
	for (i in seq(2,length(samples))){
		data=cbind(data,alldata[[i]][,c("presence","absence")])
	}

	names=c("Contig","Pos","Ref")
	for (s in names(samples)){
		names=c(names,paste(s,c("presence","absence"),sep="."))
	}
	names(data)=names
	data
}
...
add_pi2<-function(snps,samples){
	...
	for (i1 in seq(1,length(samples)-1)){
		...
		for (i2 in seq(i1+1,length(samples))){
			...
			snps[,name]=rep(0,ns)
			names1=paste(s1,c("presence","absence"),sep=".")
			names2=paste(s2,c("presence","absence"),sep=".")
			for (i in seq(1,ns)){
				sub1=snps[i,names1]
				sub2=snps[i,names2]
				sum1=rowSums(sub1)
				...
				dem=1/sum1/sum2
				dem[!is.finite(dem)]=0
				pi=sub1[,1]*sub2[,2]*dem+sub2[,1]*sub1[,2]*dem
				snps[i,name]=pi
			}
		}
	}
